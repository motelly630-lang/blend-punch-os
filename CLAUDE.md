...
- **배움 카드** — 작업 중 배운 게 생기면(대표님 지적·수정 / 실패 원인과 해결 확인 / 같은 절차 반복)
  그 작업 보고 끝에 `📚 이번에 배운 것` 카드로 알린다: 배운 것(확인/추론) · 적용 조건 · 스킬로 만들면
  **연결**(함께 쓰일 스킬·에이전트·문서) · **보완**(빈틈) · **강화**(더 좋아질 점) · "만들어도 될까요?".
  승인 전에는 스킬·에이전트·훅을 만들지 않는다. 형식은 `os-learn` §5. 배운 게 없으면 카드를 붙이지 않는다.
