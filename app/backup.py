...
def run_backup(trigger: str = "auto", triggered_by: str = "") -> Path | None:
    """DB 백업 → 로컬 저장 + S3 업로드 + 로그 기록."""
    ...
    logger.info(f"[Backup] DB 로컬 저장 → {dest} ({size_kb}KB)")
    _rotate()
    ...
    # 백업 로그 기록
    _write_log(dest.name, size_kb, "success", s3_ok, "", trigger, triggered_by)
    return dest


def _write_log(filename, size_kb, status, s3_uploaded, error_msg, trigger, triggered_by):
    """백업 결과를 DB에 기록."""
    try:
        ...
        logger.warning(f"[Backup] 로그 기록 실패: {e}")
